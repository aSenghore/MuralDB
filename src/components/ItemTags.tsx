...
interface ItemTagsProps {
    itemId: string;
    itemType: 'image' | 'document' | 'gallery' | 'folder';
    showAddButton?: boolean;
    size?: 'sm' | 'md';
    className?: string;
    galleryId?: string; // Required for image type to save tags properly
    onTagsChanged?: () => void; // Callback when tags are added/removed
}
...
export const ItemTags: React.FC<ItemTagsProps> = ({
    ...
    const handleAddTag = async (tagId: string) => {
        await addTagToItem(itemId, itemType, tagId, galleryId);
        if (onTagsChanged) {
            onTagsChanged();
        }
    };
    ...
    const handleRemoveTag = async (tagId: string) => {
        await removeTagFromItem(itemId, tagId, galleryId);
        if (onTagsChanged) {
            onTagsChanged();
        }
    };
