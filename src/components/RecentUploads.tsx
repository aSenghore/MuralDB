...
export function RecentUploads({ onNavigate, user }: RecentUploadsProps) {
  const [recentUploads, setRecentUploads] = useState<(FirebaseImage | FirebaseDocument)[]>([]);
  ...
  const handleNavigateToLocation = (file: FirebaseImage | FirebaseDocument) => {
    if (!onNavigate) return;
    
    const isImage = 'uploadedAt' in file && file.type.startsWith('image/');
    if (isImage) {
      onNavigate('art'); // Images go to art page
    } else {
                  ...
                  <div className="pt-1">
                    <div className="flex flex-wrap gap-1">
                      {file.tags.slice(0, 2).map((tag, index) => (
                        <span 
                          key={index}
                          className="inline-block bg-primary/10 text-primary text-xs px-1.5 py-0.5 rounded"
                        >
                          {tag}
                        </span>
                      ))}
